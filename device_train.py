...
def get_mask_one_locations(single_sequence):
    separator = np.where(single_sequence == 25)[0]
    separator_locations = []
    endoftext = np.where(single_sequence == 50256)[0]
   
    for endoftext_token_location in endoftext:
        locations = separator[separator <= endoftext_token_location]
        if len(locations) > 0:
            separator_locations.append(locations[-1])
        else:
            separator_locations.append(0)
    separator = np.asarray(separator_locations)
